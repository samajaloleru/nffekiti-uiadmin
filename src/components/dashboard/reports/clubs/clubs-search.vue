<template>
    <section>
        ...
        <div class="pv2 mt4 dib w-100" id="tableReport">
            ...
            <div class="cf w-100 near-black tracked tc b f3" colspan="5">CLUB REPORT</div>
                ...
                <table-display>
                    ...
                    <template slot="tableBody" v-if="recordList.length > 0">
                        <tr class="stripe-dark" v-for="(club, index) in recordList" :key="index">
                            <td class=" pa2 tr pr2">
                            <router-link data-glyph="eye" class="oi br-pill bg-green hover-bg-dark-green ph1 pt1 near-white" :to="{name:'clubs-view',params:{id:club.ID}}" >
                            </router-link>
                            </td>
                            <td class=" pa2">
                                <small class="f7">#{{index+1}}.</small>  {{club.Dob}}
                            </td>
                            <td class=" pa2 f7">{{club.Profile}}</td>
                            <td class=" pa2 f7">{{club.Phone}}</td>
                            <td class=" pa2 f7">{{club.Unit}}</td>
                            <td class=" pa2 f7">{{club.Group}}</td>
                ...
                <div v-if="recordList.length < 1">
                    <h1 class="black f6 tc ma2">There are currently no club report within the system for the period specified</h1>
                </div>
