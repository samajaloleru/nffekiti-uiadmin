<template>
		<section class="pa2 pa3-l">
			<div class="w-100 pa2 bg-light-gray br4 cf inline-flex items-center relative">
				...
				<router-link :to="{'name':'accounts-new'}" class="fl right-1 absolute ph2 pv1 br4 bg-red hover-bg-dark-red grow-ns pointer f6 white tc no-underline">
					+ New
				</router-link>
			</div>
			...
			<table-display searchBy="setting name">
				<template slot="tableHead">
					<tr class="tl bg-black black f7">
						...
						<td class="">
							<input type="text" v-model="search.filter.code" placeholder="Account No" class="ba b--black-10 f7 fl tracked bg-white black pa1 w-100 br1">
						</td>
						<td class="">
							<input type="text" v-model="search.title" placeholder="Account Name" class="ba b--black-10 f7 fl tracked bg-white black pa1 w-100 br1">
						</td>
						<td class="">
					...
					<tr class="tl bg-black white">
						<td class="pa2 "></td>
						<td class="pa2 ">Account No</td>
						<td class="pa2 ">Account Name</td>
				...
				<template slot="tableBody" v-if="recordList.length > 0">
					<tr class="stripe-dark" v-for="(setting, index) in recordList" :key="index">
						...
						<td class=" pa2 f7">{{account.Title}}</td>
						<td class=" pa2 f7">{{account.Owner}}</td>
						<td class=" pa2 f7">{{account.Partner}}</td>
						<td class=" pa2 f7">{{account.Workflow}}</td>
						<td class=" pa2 f7">{{account.Updatedate}}</td>
					</tr>
				</template>
			</table-display>

			<div v-if="recordList.length < 1">
				<p class="black f6 tc ma5">There are currently no settings</p>
			</div>
