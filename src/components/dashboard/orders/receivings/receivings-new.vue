<template>
		<section class="pa2 pa3-l">
			...
			<div class="pa2 bg-light-gray br4 cf">
                ...
                <receivinglines :refid="0" ref="receivinglines" />
...
<script type="text/javascript">
    ...
    import receivinglines from "@/components/dashboard/orders/receivings/receivings-new-lines"

	export default {
		created(){},
		data() {return{
			url: "/api/receivings",
			record: {Currency:"ZAR", Docdate: new Date().toJSON().slice(0,10),},
			notifications:[],
			partnerList:[],
			ownerList:[],
			isSave:true,
			...
			save () {
				...
				.then((response) => {
                    ...
                    if(response.data.Body !== null && response.data.Code == 200){
                        ...
                        app.$refs.receivinglines.saveLines(parseInt(response.data.Body));
                    }
				}).catch((e) => {
					console.log(e)
				})
