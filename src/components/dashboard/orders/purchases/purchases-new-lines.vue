        ...
        <table-display class="fl w-100" style="max-height:30vh">
            <template slot="tableHead" class="fixed">
                <tr class="tl bg-black white w-100">
                    ...
                    <td class="pa2 w2">Supplier</td>
                    <td class="pa2 w2">Content</td>
                    <td class="pa2 w2">Order Number</td>
                    <td class="pa2 w2">Fabric Item Order</td>
                    <td class="pa2 w2">Color</td>
                    <td class="pa2 w2">Quantity Order</td>
                    <td class="pa2 w4">Price Per Unit Excl.Vat</td>
                    <td class="pa2 w2">Vat per unit</td>
                ...
                <tr class="stripe-dark" v-for="(line, index) in recordList" :key="index">
                    ...
                    <td class=" ph1 f7">
                        <span class="items-center flex-row">
                            ...
                            <span class="ph1 br1 bg-red hover-bg-dark-red pointer white tc" @click="line.itemList=[],line.Item='',line.ItemID=0,line.Color='', line.Uom='', line.Unit=0, line.Quantity=0, line.Amount=0,line.Totalexcltax=0" >
                                CLEAR
                            </span>
                        </span>
  ...
  export default {
	...
	methods: {
        ...
        saveLines(ID) {
            const app = this;
            for (var index = 0, len = app.recordList.length; index < len; index++) {
                app.recordList[index].PricelistID = ID;
            }
			...
			HTTP.post(app.url, recordLines, {withCredentials: true}) .then((response) => {
				...
				if (response.data.Code == 200) {
                    app.$router.push({ name: "purchases-view", params: { id: ID } });
				}
			}).catch((e) => { console.log(e) })
        },
        updateParentTotal() {
