<template>
	<section class="pa2 pa3-l">
	  ...
	  <table-display>
		...
		<template slot="tableBody" v-if="recordList.length > 0">
		  <tr class="stripe-dark" v-for="(workorder, index) in recordList" :key="index">
			...
			<td class=" pa2">
			  <span class="f7">#{{(index+1)+(search.skip*search.limit)}}.</span> {{workorder.Docno}}
			</td>
			<td class=" pa2 f7">{{workorder.Docdate}}</td>
			<td class=" pa2 f7">{{workorder.Title}}</td>
			<td class=" pa2 f7">{{workorder.Owner}}</td>
			<td class=" pa2 f7">{{workorder.Workflow}}</td>
            <td class=" pa2 f7">{{workorder.Createdbyuser}} <small>{{dateTimeConvert(workorder.Createdate)}}</small> </td>
			<td class=" pa2 f7">{{workorder.Updatedbyuser}} <small>{{dateTimeConvert(workorder.Updatedate)}}</small> </td>
		  </tr>
		</template>
	  </table-display>
	  ...
	  <div v-if="recordList.length < 1">
		<h1 class="black f6 tc ma5">There are currently no workorders record </h1>
	  </div>
