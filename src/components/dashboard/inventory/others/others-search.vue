<template>
	<section class="pa2 pa3-l">
	  ...
	  <table-display>
		...
		<template slot="tableBody" v-if="recordList.length > 0">
		  <tr class="stripe-dark" v-for="(other, index) in recordList" :key="index">
			<td class="">
			  <router-link data-glyph="eye" class="f7 oi br-pill bg-green hover-bg-dark-green ph1 pt1 near-white" :to="{name:'others-view',params:{id:other.StockID}}" ></router-link>
			</td>
			<td class=" pa2">
			  <span class="f7">#{{(index+1)+(search.skip*search.limit)}}.</span> {{other.Title}}
			</td>
			<td class=" pa2 f7">{{other.Quantity}}</td>
			<td class=" pa2 f7">{{other.Amount}}</td>
			<td class=" pa2 f7">{{other.Totalexcltax}}</td>
			<td class=" pa2 f7"><small>{{other.Docno}} - {{other.Stock}} </small> </td>
			<td class="dn pa2 f7">{{other.Createdbyuser}} <small>{{dateTimeConvert(other.Createdate)}}</small> </td>
			<td class="dn pa2 f7">{{other.Updatedbyuser}} <small>{{dateTimeConvert(other.Updatedate)}}</small> </td>
		  </tr>
		</template>
	  </table-display>
	  ...
	  <div v-if="recordList.length < 1">
		<h1 class="black f6 tc ma5">There are currently no others record </h1>
	  </div>
...
<script type="text/javascript">
  ...
  export default {
	data() {return{
	  ...
	  search: {text: "", field: "Title", limit: 100, page:1, skip: 0, filter:{Code:"other"}},
	}},
