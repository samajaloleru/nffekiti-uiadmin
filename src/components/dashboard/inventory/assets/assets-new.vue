			...
			<div class="pa2 bg-light-gray br4 cf">
				...
				<div class="w-100">
					...
					<div class="fl pa2 mt3 mt0-ns w-100 w-third-l">
						<label class="db fw4 lh-copy f6 black"> Asset Title</label>
						<input class="pa2 ba b--silver br2 bg-white w-100 " type="text" v-model="record.Title">
					</div>
				</div>

                <assetlines :refid="0" ref="assetlines" />
...
<script type="text/javascript">
    ...
    import assetlines from "@/components/dashboard/inventory/assets/assets-new-lines"

	export default {
		created(){},
		data() {return{
			url: "/api/stocks",
			record: {Code:"asset", Currency:"ZAR", Workflow:"draft", Docdate: new Date().toISOString().slice(0,10), Totalincltax:0, Taxamount:0, Totalexcltax:0, Discountamount:0},
			notifications:[],
			partnerList:[],
			ownerList:[],
			isSave:true,
			...
			save () {
				...
				.then((response) => {
					...
					if(response.data.Body !== null && response.data.Code == 200){
                        ...
                        app.$refs.assetlines.saveLines(parseInt(response.data.Body));
					}
				}).catch((e) => {
					console.log(e)
				})
