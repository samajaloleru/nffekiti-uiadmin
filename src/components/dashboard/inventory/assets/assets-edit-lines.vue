...
<script type="text/javascript">
  ...
  export default {
	...
	methods: {
		...
		addItem(){
            const app = this
			var curLine = {ID:0, Code:"asset", StockID:parseInt(app.refid), Checked:false, Lineno:app.recordList.length+1, Quantity:1, Item:"", UOM:"", ItemID:0, Amount:0, Totalexcltax:0, itemList:[]}
		...
		saveLines(){
			...
			HTTP.post(app.url, recordLines, {withCredentials: true}) .then((response) => {
				...
				if (response.data.Code == 200) {
					app.$router.push({name:"assets-view",params:{id:parseInt(app.refid)}})
				}
			}).catch((e) => { console.log(e) })
        },
        updateParentTotal(){
